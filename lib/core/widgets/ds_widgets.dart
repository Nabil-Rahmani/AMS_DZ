...
class DarkAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  const DarkAppBar({
    ...
    this.actions, this.leading, this.hasGradient = true,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: AppBar(
        ...
        leading: leading,
        title: showLogo ? const AmsLogo(size: 36) : (title != null ? Text(title!) : null),
        actions: actions,
        centerTitle: false,
      ),
    );
...
class GradientButton extends StatefulWidget {
  ...
  const GradientButton({
    ...
    this.isLoading = false, this.icon, this.height = 56,
    this.isGold = false, this.width,
  });
...
class DarkTextField extends StatefulWidget {
  ...
  const DarkTextField({
    ...
    this.validator, this.textDir, this.suffix,
  });
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(16),
        boxShadow: _focused
            ? [BoxShadow(color: DS.purpleGlow, blurRadius: 20, spreadRadius: 2)]
            : [],
      ),
      child: TextFormField(
...
class DarkCard extends StatefulWidget {
  ...
  const DarkCard({
    ...
    this.radius = 20, this.shadow, this.margin,
  });
...
class DarkBadge extends StatelessWidget {
  ...
  const DarkBadge({
    ...
    this.dot = false, this.pulse = false,
  });
    ...
    return AnimatedBuilder(
      ...
      builder: (_, __) => Container(
        ...
        decoration: BoxDecoration(
          ...
          color: widget.color.withValues(alpha: widget.animate ? _anim.value : 1.0),
        ),
      ),
    );
  }
...
class DSSection extends StatelessWidget {
  ...
  const DSSection({super.key, required this.title, this.trailing, this.onTrailing});
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: DS.purple.withValues(alpha: 0.1), width: 2),
                ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Center(child: Text(initials, style: TextStyle(
        fontSize: radius * 0.55, fontWeight: FontWeight.w800, color: Colors.white,
      ))),
    );
  }
}
...
class DSStatCard extends StatelessWidget {
  ...
  const DSStatCard({
    ...
    required this.value, required this.color, this.loading = false,
  });
...
class DSBanner extends StatelessWidget {
  ...
  const DSBanner({
    super.key, required this.message, required this.color, required this.icon, this.action,
  });
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Text(message, style: TextStyle(
            color: color, fontWeight: FontWeight.w600, fontSize: 13, height: 1.4,
          ))),
        ]),
...
class PurpleOrb extends StatelessWidget {
  ...
  const PurpleOrb({
    super.key, this.size = 340,
    this.alignment = Alignment.topRight, this.opacity = 1.0,
  });
    ...
    return Align(
      ...
      child: Transform.translate(
        ...
        child: Opacity(
          ...
          child: Container(
            width: size, height: size,
            decoration: const BoxDecoration(shape: BoxShape.circle, gradient: DS.orbGradient),
          ),
        ),
      ),
    );
...
class DSPriceTag extends StatelessWidget {
  ...
  const DSPriceTag({
    ...
    this.isGold = false, this.large = false,
  });
...
class DSCountdown extends StatelessWidget {
  ...
  const DSCountdown({super.key, required this.time, this.isUrgent = false, this.large = false});
    ...
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(time, style: TextStyle(
          fontWeight: FontWeight.w800, fontSize: large ? 20 : 13,
