...
class ProfilePage extends StatelessWidget {
                  ...
                  child: Column(
                    children: [
                      ...
                      FadeSlideIn(delay: const Duration(milliseconds: 100),
                        child: Text(user.name, style: DS.displayLarge.copyWith(fontSize: 28))),
                      const SizedBox(height: 4),
                      FadeSlideIn(delay: const Duration(milliseconds: 150),
                        child: Text(user.email, style: DS.body.copyWith(color: DS.textSecondary))),
                      
                      const SizedBox(height: 12),
                      FadeSlideIn(delay: const Duration(milliseconds: 200),
                        child: DarkBadge(
                          label: user.role == UserRole.organizer ? 'منظم مبيعات' : 'مزايد معتمد',
                          color: user.role == UserRole.organizer ? DS.success : DS.purple,
                        )),
                    ],
                  ),
                ),
                ...
                sliver: SliverList(
                  delegate: SliverChildListDelegate([
                    FadeSlideIn(delay: const Duration(milliseconds: 250),
                      child: const DSSection(title: 'المعلومات الشخصية')),
                    ...
                    FadeSlideIn(
                      ...
                      child: TapAnimated(
                        ...
                        child: GlassCard(
                          ...
                          border: Border.all(color: DS.error.withValues(alpha: 0.15)),
                          child: Row(
                            children: [
                              Container(
                                width: 44, height: 44,
                                ...
                                child: const Icon(Icons.logout_rounded, color: DS.error, size: 22),
                              ),
                              const SizedBox(width: 16),
                              Expanded(
                                child: Text(
                                  'تسجيل الخروج',
                                  style: DS.titleS.copyWith(color: DS.error, fontWeight: FontWeight.w700),
                                ),
                              ),
                              const Icon(Icons.arrow_back_ios_new_rounded, color: DS.error, size: 14),
                            ],
                          ),
                        ),
                      ),
  ...
  Widget _buildInfoCard(String label, String value, IconData icon, Color color, {int delay = 0}) {
    return FadeSlideIn(
  ...
  String _kycLabel(KycStatus? status) {
    switch (status) {
      case KycStatus.approved: return 'موثق';
      case KycStatus.pending: return 'قيد المراجعة';
      case KycStatus.rejected: return 'مرفوض';
      default: return 'لم يتم التقديم';
    }
  }

  Color _kycColor(KycStatus? status) {
    switch (status) {
      case KycStatus.approved: return DS.success;
      case KycStatus.pending: return DS.warning;
      case KycStatus.rejected: return DS.error;
