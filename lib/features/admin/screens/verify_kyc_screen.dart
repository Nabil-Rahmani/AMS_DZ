...
class _VerifyKycScreenState extends State<VerifyKycScreen>
  ...
  Future<void> _openDocument(String urlOrPath) async {
    try {
      ...
      if (!urlOrPath.startsWith('http')) {
        downloadUrl =
            await FirebaseStorage.instance.ref(urlOrPath).getDownloadURL();
      }
                        ...
                        child: Row(children: [
                          ...
                          Expanded(
                            child: Text(
                              _docLabel(e.key),
                              style:
                                  DS.bodySmall.copyWith(color: DS.textPrimary),
                            ),
                          ),
                          TextButton(
  ...
  String _docLabel(String key) {
    switch (key) {
      case 'national_id':
        return 'بطاقة التعريف الوطنية';
      case 'commercial_register':
        return 'السجل التجاري';
      case 'product_docs':
        return 'وثائق المنتج';
      case 'no_lien_certificate':
        return 'شهادة عدم الرهن';
      default:
        return key;
    }
  }
