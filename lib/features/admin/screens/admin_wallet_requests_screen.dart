              ...
              child: Padding(
                ...
                child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                  Container(
                    ...
                    child: const Icon(Icons.account_balance_rounded, color: DS.purple, size: 22),
                  ),
                  const SizedBox(width: 14),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('طلبات الشحن', style: DS.titleL.copyWith(fontSize: 22)),
                    Text('المعلقة بانتظار الموافقة', style: DS.bodySmall),
                  ]),
                ]),
              ),
            ),
...
class _DepositRequestCardState extends State<_DepositRequestCard> {
  ...
  Future<void> _reject() async {
    ...
    try {
      ...
      await widget.db.rejectDeposit(widget.tx.id, widget.tx.userId, widget.tx.amount);
      _snack('تم رفض الطلب');
    } catch (e) {
  ...
  Future<void> _approve() async {
    setState(() => _loading = true);
    try {
      await widget.db.approveDeposit(widget.tx.id, widget.tx.userId, widget.tx.amount);
      _snack('تمت الموافقة ✅');
    } catch (e) {
      _snack('خطأ: $e', isError: true);
    } finally {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Container(
            ...
            child: const Icon(Icons.account_balance_wallet_rounded, color: DS.purple, size: 18),
          ),
