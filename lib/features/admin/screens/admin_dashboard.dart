...
class _AdminDashboardState extends State<AdminDashboard>
  ...
  Widget _buildSidebar() {
    return GlassCard(
      ...
      child: SizedBox(width: 260, child: Column(children: [
        Container(
          height: 180, clipBehavior: Clip.antiAlias,
          decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: DS.border))),
          child: Stack(children: [
            const Positioned(top: -40, left: -40, child: PurpleOrb(size: 150, opacity: 0.2)),
            Padding(
              padding: const EdgeInsets.fromLTRB(24, 60, 24, 24),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const AmsLogo(size: 42), const SizedBox(height: 18),
                Text('لوحة التحكم', style: DS.titleM),
                const SizedBox(height: 4),
                Text('الإدارة العامة للنظام', style: DS.bodySmall.copyWith(color: DS.textMuted)),
              ]),
            ),
          ]),
        ),
