...
class _AuctionDetailScreenState extends State<AuctionDetailScreen>
  ...
  Future<void> _placeBid(AuctionModel auction) async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('✅ تم تسجيل المزايدة بنجاح'), backgroundColor: Colors.green),
      );
    } catch (e) {
  ...
  List<String> _buildImageList(AuctionModel auction) {
    ...
    if (auction.imageUrl != null && auction.imageUrl!.isNotEmpty) return [auction.imageUrl!];
    return [];
  }
                              ...
                              child: Row(children: [
                                ...
                                Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                                  ...
                                  if (isUpcoming && auction.startTime != null) ...[
                                    const SizedBox(height: 10),
                                    _DetailUpcomingTimer(startTime: auction.startTime!),
                                  ],
                                ]),
                              ]),
                            ),
              ...
              if (isActive)
                StreamBuilder<DocumentSnapshot>(
                  ...
                  builder: (_, auctionSnap) {
                    ...
                    final currentAuction  = AuctionModel.fromMap(data, widget.auctionId);
                      ...
                      return Positioned(
                        ...
                        child: Container(
                          ...
                          child: GradientButton(
                            ...
                            height: 54,
                            onPressed: _depositPaying ? null : () => _payDeposit(currentAuction),
                          ),
                        ),
                      );
                    }
                      ...
                      Container(
                        ...
                        child: Row(children: [
                          Container(
                            ...
                            child: const Icon(Icons.schedule_rounded, color: DS.purple, size: 20),
                          ),
                          const SizedBox(width: 12),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            ...
                            if (auction.startTime != null)
                              _UpcomingCountdownText(startTime: auction.startTime!),
                          ])),
                        ]),
                      ),
                    ]),
                  ...
                  if (_method == 'bank') ...[
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        labelText:  'اسم البنك',
                        prefixIcon: const Icon(Icons.account_balance_rounded, color: DS.gold),
                        filled:     true,
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        prefixIcon: const Icon(Icons.credit_card_rounded, color: DS.gold),
                        filled:     true,
                        fillColor:  DS.bgField,
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(12),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 3),
          Text(subtitle, style: const TextStyle(fontSize: 10, color: DS.textMuted)),
        ]),
      ),
    );
  }
...
class _UpcomingCountdownTextState extends State<_UpcomingCountdownText> {
  ...
  void _update() {
    ...
    if (_rem == Duration.zero) { _timer?.cancel(); _timer = null; }
  }
  ...
  Widget build(BuildContext context) {
    if (_rem == Duration.zero) {
      return Text('يبدأ الآن...', style: DS.bodySmall.copyWith(color: DS.success));
    }
    final d = _rem.inDays;
    ...
    final color     = isUrgent ? DS.error : isWarning ? const Color(0xFFF59E0B) : DS.textSecondary;
    return Text(timeText,
        style: DS.bodySmall.copyWith(color: color, fontWeight: FontWeight.w600));
  }
}
...
class _DetailUpcomingTimerState extends State<_DetailUpcomingTimer> {
  ...
  Widget build(BuildContext context) {
    ...
    final isUrgent  = _rem.inMinutes < 5;
    final isWarning = _rem.inMinutes < 30 && !isUrgent;
    final color     = isUrgent ? DS.error : isWarning ? const Color(0xFFF59E0B) : DS.purple;
...
class _CategoryDetailsWidget extends StatelessWidget {
  ...
  String _translateKey(String key) {
    const map = {
      'brand': 'الماركة', 'model': 'الموديل', 'year': 'سنة الصنع',
      'engine': 'المحرك', 'fuel': 'الوقود', 'mileage': 'المسافة',
      'transmission': 'ناقل الحركة', 'color': 'اللون', 'condition': 'الحالة',
      'vehicleType': 'نوع المركبة', 'technicalVisit': 'الفحص التقني',
      'type': 'النوع', 'location': 'الموقع', 'area': 'المساحة',
      'rooms': 'عدد الغرف', 'floor': 'الطابق', 'age': 'عمر البناء',
      'ram': 'الذاكرة', 'storage': 'التخزين', 'processor': 'المعالج',
      'screen': 'الشاشة', 'material': 'المادة', 'dimensions': 'الأبعاد',
      'weight': 'الوزن', 'carat': 'العيار', 'description': 'الوصف',
      'quantity': 'الكمية', 'origin': 'المنشأ', 'warranty': 'الضمان',
    };
    return map[key] ?? key;
  }
  ...
  _CategoryConfig _getCategoryConfig(String category) {
    ...
    if (cat.contains('سيارة') || cat.contains('مركبة') || cat.contains('مركبات') || cat.contains('car')) {
      return _CategoryConfig(icon: Icons.directions_car_rounded, title: 'معلومات السيارة', fields: [
        _Field('brand', '🚘', 'الماركة'), _Field('model', '🚗', 'الموديل'),
        _Field('year', '📅', 'سنة الصنع'), _Field('engine', '⚙️', 'المحرك'),
        _Field('fuel', '⛽', 'الوقود'), _Field('mileage', '📏', 'المسافة', unit: 'كم'),
        _Field('transmission', '🔄', 'ناقل الحركة'), _Field('color', '🎨', 'اللون'),
        _Field('condition', '✅', 'الحالة'), _Field('vehicleType', '🚙', 'نوع المركبة'),
        _Field('technicalVisit', '🔧', 'الفحص التقني'),
      ]);
    }
    if (cat.contains('عقار') || cat.contains('شقة') || cat.contains('بيت')) {
      return _CategoryConfig(icon: Icons.home_rounded, title: 'معلومات العقار', fields: [
        _Field('type', '🏢', 'النوع'), _Field('location', '📍', 'الموقع'),
        _Field('area', '📐', 'المساحة', unit: 'م²'), _Field('rooms', '🛏', 'عدد الغرف'),
        _Field('floor', '🏗', 'الطابق'), _Field('age', '📅', 'عمر البناء', unit: 'سنة'),
        _Field('condition', '✅', 'الحالة'),
      ]);
    }
    if (cat.contains('إلكترون') || cat.contains('جهاز') || cat.contains('هاتف')) {
      return _CategoryConfig(icon: Icons.devices_rounded, title: 'المواصفات التقنية', fields: [
        _Field('type', '🖥', 'النوع'), _Field('brand', '🏷', 'الماركة'),
        _Field('model', '📱', 'الموديل'), _Field('ram', '🧠', 'RAM', unit: 'GB'),
        _Field('storage', '💾', 'التخزين'), _Field('processor', '⚡', 'المعالج'),
        _Field('screen', '🖥', 'الشاشة'), _Field('condition', '✅', 'الحالة'),
      ]);
    }
    if (cat.contains('أثاث')) {
      return _CategoryConfig(icon: Icons.chair_rounded, title: 'معلومات الأثاث', fields: [
        _Field('type', '🪑', 'النوع'), _Field('material', '🪵', 'المادة'),
        _Field('color', '🎨', 'اللون'), _Field('dimensions', '📐', 'الأبعاد'),
        _Field('condition', '✅', 'الحالة'),
      ]);
    }
    if (cat.contains('مجوهرات') || cat.contains('ذهب')) {
      return _CategoryConfig(icon: Icons.diamond_rounded, title: 'معلومات المجوهرات', fields: [
        _Field('type', '💍', 'النوع'), _Field('material', '✨', 'المادة'),
        _Field('weight', '⚖️', 'الوزن', unit: 'غ'), _Field('carat', '💎', 'العيار'),
        _Field('condition', '✅', 'الحالة'),
      ]);
    }
    return _CategoryConfig(
      ...
      title:  'تفاصيل إضافية',
      fields: details.keys.map((k) => _Field(k, 'ℹ️', _translateKey(k))).toList(),
    );
  }
}
...
class _Field {
  final String key, emoji, label; final String? unit;
