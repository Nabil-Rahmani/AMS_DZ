    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: isWinner ? DS.goldDark : DS.border,
              width: isWinner ? 2 : 1),
          boxShadow: [
            BoxShadow(
                color: isWinner
                    ? DS.goldDark.withValues(alpha: 0.12)
            ...
            child: Row(children: [
              Expanded(
                  child: Column(
                      ...
                      children: [
                    Text(auction?.title ?? 'مزاد غير معروف',
          ...
          Padding(
            ...
            child:
              ...
              Row(children: [
                Expanded(
                    child: _SmallTile(
                        label: 'أعلى مزايدة منك',
                        value: '${highestBid.toStringAsFixed(0)} DZD',
                        highlight: true)),
                const SizedBox(width: 12),
                if (auction != null)
                  Expanded(
                      child: _SmallTile(
                          label: 'السعر الحالي',
                          value:
                              '${(auction!.currentPrice ?? 0).toStringAsFixed(0)} DZD')),
              ]),
              const SizedBox(height: 14),
              Text('مزايداتك (${bids.length})',
                  style: const TextStyle(
                      fontWeight: FontWeight.w700,
                      fontSize: 13,
                      color: DS.textPrimary)),
                    ...
                    child: Row(children: [
                      Container(
                          ...
                          decoration: const BoxDecoration(
                              color: DS.purple, shape: BoxShape.circle)),
                      const SizedBox(width: 8),
                      Text('${bid.amount.toStringAsFixed(0)} DZD',
                          style: const TextStyle(
                              fontWeight: FontWeight.w700,
                              fontSize: 13,
                              color: DS.textPrimary)),
                      ...
                      if (bid.timestamp != null)
                        Text(_fmt(bid.timestamp!),
                            style: const TextStyle(
                                fontSize: 11, color: DS.textSecondary)),
                    ]),
                  )),
              if (bids.length > 3)
                Text('+ ${bids.length - 3} مزايدات أخرى',
                    style: const TextStyle(
                        fontSize: 12,
                        color: DS.purple,
  ...
  Widget _buildDarkBadge(AuctionStatus status) {
    switch (status) {
      case AuctionStatus.active:
        return const DarkBadge(label: 'مباشر', color: DS.success, dot: true);
      case AuctionStatus.ended:
        return const DarkBadge(label: 'منتهي', color: DS.textMuted);
      case AuctionStatus.approved:
        return const DarkBadge(label: 'قادم', color: DS.info);
      default:
        return DarkBadge(label: status.name, color: DS.textMuted);
    }
  }
}
