                              ...
                              GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                                    ...
                                    Text('شحن رصيد', style: DS.label.copyWith(color: Colors.white, fontSize: 13)),
                                  ]),
                                ),
                              ),
                            ]),
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                gradient: const LinearGradient(
                                  colors: [Color(0xFF1A2340), Color(0xFF0D1528)],
                              ...
                              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                ...
                                Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
                                  Text(
                                    ...
                                    style: const TextStyle(fontSize: 44, fontWeight: FontWeight.w800, color: Colors.white, letterSpacing: -1),
                                  ),
                                ...
                                Row(children: [
                                  Container(
                                    ...
                                    child: const Icon(Icons.lock_rounded, color: Color(0xFFF59E0B), size: 14),
                                  ),
                                  const SizedBox(width: 10),
                                  Text('الضمان المحجوز:', style: DS.bodySmall.copyWith(color: Colors.white54)),
                                  const Spacer(),
                                  Text(
                                    'DZD ${user != null ? user.blockedBalance.toStringAsFixed(0) : '0'}',
                                    style: DS.titleS.copyWith(color: const Color(0xFFF59E0B), fontSize: 14),
                                  ),
                                ]),
                              ]),
                            ),
  ...
  Widget build(BuildContext context) => GlassCard(
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text('${tx.createdAt.day}/${tx.createdAt.month}/${tx.createdAt.year}', style: DS.bodySmall),
      ])),
      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
        Text('+${tx.amount.toStringAsFixed(0)} دج', style: DS.titleM.copyWith(color: DS.success, fontSize: 15)),
        ...
        Container(
          ...
          child: Text(_statusLabel, style: DS.bodySmall.copyWith(color: _statusColor, fontSize: 11)),
        ),
      ]),
    ]),
  );
...
class _RechargeSheetState extends State<RechargeSheet> {
  ...
  @override
  void dispose() {
    _amountCtrl.dispose(); _cardNameCtrl.dispose();
    _cardNumberCtrl.dispose(); _cardExpiryCtrl.dispose(); _cardCvvCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _pickAndUploadProof() async {
    ...
    try {
      ...
      final ref = FirebaseStorage.instance.ref('deposit_proofs/${widget.uid}/${DateTime.now().millisecondsSinceEpoch}.jpg');
  ...
  Future<void> _submit() async {
    ...
    try {
      if (_method == _PayMethod.ccp) {
        ...
        await widget.db.requestDeposit(userId: widget.uid, amount: amount, method: DepositMethod.ccp, proofUrl: _proofUrl);
        if (mounted) { Navigator.pop(context); _snack('تم إرسال طلب الشحن، بانتظار موافقة المدير ⏳'); }
      } else {
        final amount = double.tryParse(_amountCtrl.text);
        if (amount == null || amount <= 0) throw Exception('المبلغ غير صالح');
              ...
              child: BackdropFilter(
                ...
                child: Container(
                  ...
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    Container(width: 64, height: 64, decoration: BoxDecoration(shape: BoxShape.circle, color: DS.purple.withValues(alpha: 0.1), border: Border.all(color: DS.purple.withValues(alpha: 0.3))), child: const Icon(Icons.sms_rounded, color: DS.purple, size: 30)),
                    const SizedBox(height: 16),
                    Text('تحقق من الدفع', style: DS.titleM),
                    const SizedBox(height: 8),
                    Text('تم إرسال رمز التحقق إلى هاتفك', style: DS.body, textAlign: TextAlign.center),
                    const SizedBox(height: 12),
                    Container(
                      width: double.infinity, padding: const EdgeInsets.all(14),
                      decoration: BoxDecoration(color: DS.bgElevated, borderRadius: BorderRadius.circular(14), border: Border.all(color: DS.purple.withValues(alpha: 0.2))),
                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: DS.purple.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.credit_card_rounded, color: DS.purple, size: 18)),
                        const SizedBox(width: 10),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text('CIB Algérie', style: DS.label),
                          const SizedBox(height: 4),
                          RichText(text: TextSpan(style: DS.body, children: [const TextSpan(text: 'رمز التحقق: '), TextSpan(text: _generatedOtp, style: DS.titleM.copyWith(color: DS.purple, letterSpacing: 4))])),
                          const SizedBox(height: 2),
                          Text('صالح لمدة 5 دقائق', style: DS.bodySmall.copyWith(color: DS.textMuted)),
                        ])),
                      ]),
                    ),
                    const SizedBox(height: 16),
...
class _MethodTab extends StatelessWidget {
  ...
  const _MethodTab({required this.label, required this.subtitle, required this.icon, required this.selected, required this.onTap});
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: selected ? DS.purple : DS.border, width: 1.5),
        boxShadow: selected ? DS.purpleShadow : null,
      ),
      child: Column(children: [
        Icon(icon, color: selected ? Colors.white : DS.textSecondary, size: 26),
        const SizedBox(height: 8),
        Text(label, style: DS.titleS.copyWith(color: selected ? Colors.white : DS.textPrimary)),
        const SizedBox(height: 2),
        Text(subtitle, style: DS.bodySmall.copyWith(color: selected ? Colors.white.withValues(alpha: 0.7) : DS.textMuted, fontSize: 11)),
      ]),
    ),
  );
}
...
class _CcpInfoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: DS.bgElevated, borderRadius: BorderRadius.circular(16), border: Border.all(color: DS.border)),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
...
class _InfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(children: [
      ...
      Expanded(child: Text(value, style: DS.bodySmall.copyWith(color: DS.textPrimary, fontWeight: FontWeight.w700))),
      GestureDetector(
...
class _ProofUploadButton extends StatelessWidget {
  ...
  final VoidCallback onTap;
  const _ProofUploadButton({required this.proofUrl, required this.uploading, required this.onTap});
  @override
  Widget build(BuildContext context) => GestureDetector(
