...
class _OrganizerDashboardHomeState extends State<OrganizerDashboardHome>
  ...
  Future<void> _loadUserData() async {
    ...
    if (doc.exists) {
      ...
      setState(() {
        ...
        _hasUploadedDocs = d['kycDocuments'] != null && (d['kycDocuments'] as Map).isNotEmpty;
        _userName = d['name'] ?? '';
      });
    }
    final snap = await _db.collection('auctions').where('organizerId', isEqualTo: uid).get();
  ...
  String _fmt(DateTime? d) { if (d == null) return '—'; return '${d.day}/${d.month}/${d.year}'; }
              ...
              flexibleSpace: FlexibleSpaceBar(
                background: Stack(
                  ...
                  children: [
                    ...
                    const PurpleOrb(size: 320, alignment: Alignment.topRight, opacity: 0.1),
                    const PurpleOrb(size: 240, alignment: Alignment.bottomLeft, opacity: 0.05),
                    ClipRRect(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            FadeSlideIn(
                              ...
                              child: Text(
                                firstName.isEmpty ? 'أهلاً بك 👋' : 'أهلاً، $firstName 👋',
                  ...
                  child: FadeSlideIn(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: DS.purple.withValues(alpha: 0.2), width: 1.5),
                        boxShadow: [
                          BoxShadow(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintStyle: TextStyle(color: DS.textMuted.withValues(alpha: 0.6), fontSize: 14),
  ...
  Widget _buildKycBanner() {
    ...
    if (_kycStatus == KycStatus.rejected) {
      return KycStatusCard(
        ...
        title: 'تم رفض طلبك',
        subtitle: _kycRejectionReason != null ? 'السبب: $_kycRejectionReason' : 'ارفع وثائق جديدة',
        tag: 'مرفوض',
    ...
    return KycStatusCard(
      ...
      title: _hasUploadedDocs ? 'وثائقك قيد المراجعة' : 'التحقق من الهوية مطلوب',
      ...
      action: OutlinedButton.icon(
        ...
        icon: Icon(_hasUploadedDocs ? Icons.edit_document : Icons.upload_file_rounded, size: 16),
  ...
  Widget _buildAuctionCard(AuctionModel auction) {
    ...
    switch (auction.status) {
      case AuctionStatus.active:    color = DS.success; label = 'نشط'; break;
      case AuctionStatus.approved:  color = DS.purple;  label = 'مقبول'; break;
      case AuctionStatus.rejected:  color = DS.error;   label = 'مرفوض'; break;
      case AuctionStatus.ended:     color = DS.textMuted; label = 'منتهي'; break;
      case AuctionStatus.submitted: color = DS.warning; label = 'قيد المراجعة'; break;
      default:                      color = DS.textMuted; label = 'مسودة';
    }
    ...
    return AuctionTapCard(
      ...
      statusColor: color,
      statusLabel: label,
      onTap: () => Navigator.pushNamed(context, AppRoutes.trackAuction, arguments: auction),
      dateFormatter: _fmt,
    );
  }
}
